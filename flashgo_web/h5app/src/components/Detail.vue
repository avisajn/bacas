<template>
  <div v-if="dealDetail != null" class="content">
    <download-banner :show="showDownloadBanner" @closeDownload="closeDownload"></download-banner>
    <!-- Slider -->
    <div class="swiper-out-container">
      ...
      <swiper :options="swiperOption">
        <template v-for="image in dealDetail.dealarticleimages">
          <swiper-slide>
    ...
    <div class="price-block">
      ...
      <div class="row">
        <span v-if="dealDetail.deal.current_price<dealDetail.deal.original_price && dealDetail.deal.original_price>0 "
              class="pull-left original-price">Rp {{dealDetail.deal.original_price | formatMoney}}</span>
        <span v-else class="pull-left original-price">Rp ???</span>
        <span v-if="dealDetail.deal.sales>0"
              class="pull-right sales">Habis: {{dealDetail.deal.sales | formatMoney}}</span>
      </div>
    </div>
    ...
    <div v-if="dealDetail.deal.stars>0 && dealDetail.deal.comments>0" class="row review-block">
      <template v-for="i in 5">
        <img v-if="dealDetail.deal.stars - i >= 1" src="http://kasbon.cash/ads/flashgo/static/images/star_one.png"
             class="pull-left star-icon">
        <img v-if="dealDetail.deal.stars - i > 0 && dealDetail.deal.stars - i < 1"
             src="http://kasbon.cash/ads/flashgo/static/images/star_half.png"
             class="pull-left star-icon">
        <img v-if="dealDetail.deal.stars - i <= 0" src="http://kasbon.cash/ads/flashgo/static/images/star_zero.png"
             class="pull-left star-icon">
      </template>
    ...
    <div v-if="relativeDeals.length>0" class="row inline-scroll">
      <div style="width: 200rem;">
        <a v-for="relativeDeal in relativeDeals" href="#" @click.prevent="goToDetail(relativeDeal.deal.id)">
    ...
    <template v-if="dealDetail.deal.type==='F' && !ongoing">
      <a href="#" @click.prevent="download">
    ...
    <template v-else>
      <a href="#" @click.prevent="toTarget(dealDetail.deal.trackinglink)">
...
<script>
  ...
  export default {
    name: 'Detail',
    components: {DownloadBanner, Countdown, swiper, swiperSlide},
    data() {
      return {
        dealId: 0,
        dealDetail: null,
    ...
    mounted() {
      let self = this
      self.dealId = self.$route.params.dealId
      self.initData()
    },
    methods: {
      initData() {
        let self = this
        Common.apiRequest('POST', 'sales/deals/get_deal_detail/', {
          user_id: 'test',
          deal_id: parseInt(self.dealId)
        }, function (data) {
          self.dealDetail = data.data
          if (self.dealDetail.deal.type === 'F') {
            self.ongoing = (new Date(self.dealDetail.flash.now).getTime() > new Date(self.dealDetail.flash.starttime).getTime())
          }
        }, true, false)
        Common.apiRequest('POST', 'sales/deals/get_relative_recommendation/', {
          user_id: 'test',
          page_id: 1,
          count: 8,
          deal_id: parseInt(self.dealId)
        }, function (data) {
          self.relativeDeals = data.data
        }, true, false)
      },
      back() {
        BacaAndroid.goBack();
      ...
      goToDetail(dealId) {
        ...
        this.$router.replace({path: '/detail/' + dealId})
      },
      download() {
