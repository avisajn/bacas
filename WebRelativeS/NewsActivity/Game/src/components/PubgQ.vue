<template>
  <div class="full">
    <pubg-step :dataSource="optionData" :stepConfig="config" @change="selectedFn"></pubg-step>
  </div>
</template>
<script>
export default {
      ...
      stepDataSource: [{ //1
        ...
        { //3
          ...
          options: [
            ...
            { label: '7,5,6', letter: 'D', text: 'Baku tembak jarak dekat' },
        ...
        { //5
          ...
          options: [
            { label: '1,2', letter: 'A', text: 'Chicken Dinner' },
    ...
    selectedFn(item) {
      ...
      setTimeout(() => {
        if (_self.config.currStepIndex == _self.config.total) {
          _self.$router.replace({ name: 'Enname', params: { data: _self.dataArray, source: 'pubg' } });
        } else {
