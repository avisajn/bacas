...
$(function(){
    ...
    // modal 的关闭
    $('.modal-body').on(eventType ,'.close,.btn-close' ,function () {
        const $parent = $(this).parents('.modal-body');
        $parent.addClass('a-fadeoutT');
        setTimeout(function () {
            $parent.removeClass('a-fadeoutT');
            $body.removeClass('container-fixed').removeClass('container-modal');
    ...
    $('[share-fb]').click(function(){
        window.showShare('facebook');
    });

    $('[share-tw]').click(function(){
        window.showShare('twitter');
    });
    $('[share-wp]').click(function(){
        window.showShare('whatsapp');
    });


});
$('[downapp]').click(function(){
    window.Download.redirect('detail');
});
...
// toast 相关 - by ymark
;(function(win){
    const $toast = $('<div id="My_toast_'+(new Date().getTime())+'" class="toasted-container top-center"></div>');
    var _timeout = null;
    var hide_fun = null;
    // 要求最短展示2秒，两秒种之后，触摸屏幕任意事件，都可以关闭
    var _is_lock = false;

    $('body').append($toast).on('click' ,function(){
        !_is_lock && typeof(hide_fun) == 'function' && hide_fun() ;
    });

    const default_config = {
        time : 5,
        type : 'message',   // message ,loading
        width : 'auto',
    }
    const template_data = {
        message : '<div class="toasted animate primary default" style="width:auto;">{content}</div>',
    ...
    win.toast = function(title ,param) {
        _timeout && clearTimeout(_timeout);
        hide_fun ? hide_fun = null : '';
        _is_lock = true;

        console.log('param:',param);
        param = mergeObj(default_config ,param);
        const $toast_content = $((template_data[param.type]||'').replace('{content}' ,title).replace('width:auto','width:'+(param.width!='auto'?param.width+'px':'auto')));
        $toast.html($toast_content);
        hide_fun = function(){
            $toast_content.removeClass('animate').addClass('animate-over');
            _timeout = setTimeout(function(){
                $toast.html('');
                _timeout = null;
            },0.3 * 1000);
        }
        _timeout = setTimeout(function(){ _is_lock = false } ,2000);
        _timeout = setTimeout(hide_fun,param.time * 1000);
        return hide_fun;
    }
