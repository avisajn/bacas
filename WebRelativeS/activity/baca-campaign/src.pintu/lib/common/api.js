...
module.exports = {
  ...
  getUserInfo : function (resolve) {
    getData({url : '/activity/pt/user' ,method:'post' ,param:{t:token}} ,resolve);
  },

  // 得到一个用户
  getRanking : function(resolve){
    getData({url : '/activity/pt/ranking' ,method:'get'} ,resolve);
  },

  addUser:function(resolve){
    getData({url : '/activity/pt/adduser' ,method:'post' ,param:{
      name : $('#form_name').val(),
  ...
  sub : function(time ,resolve){
    getData({url : '/activity/pt/add' ,method:'post' ,param:{
      t : encryptCode({
        t : token ,
        time : time,
        name : $('#form_name').val(),
