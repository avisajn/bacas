...
var oldthumbImgUrl = Country['imgHostOld']+'api/v1/NewsImage/{`url`}?allowRedirect=true&thumbnail=true'; //缩略图：
// http://img.cdn.baca.co.id/4f29a652-9b99-4265-84e5-1061e2a6c9e4_thumbnail
// http://img.cdn.cennoticias.com/9a94dfbd-8bab-4f55-8a6a-7caf5c63d33c_thumbnail
// http://img.cdn.menanip.com/0fdcc092-74d3-4bb4-90ff-2594901e6261_thumbnail
...
module.exports = {
	item : {
	    ...
	    _getImg : function(src){
	    	// return '<img class="lazyload-img" a="1" onError="window.onImgError(this);" old-src="'+oldthumbImgUrl.replace('{`url`}',src)+'" onload="lzld(this)" src="" data-src="'+thumbImgUrl.replace('{`url`}',src)+'" />';
	    	return '<img class="lazyload-img" a="1" onError="window.onImgError(this);" old-src="'+oldthumbImgUrl.replace('{`url`}',src)+'" onload="lzld(this)" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="'+thumbImgUrl.replace('{`url`}',src)+'" />';
	    },
	    // i : 指的是第几个，方便统计用
		  // m : 指的是一共多少个
	    ...
	    _getAtlasItem:function(k, i, origin){
	        var html = [];
	        ...
	        html.push('  <a class="item-link clearfix " href="javascript:window.Link.toAtlas(\''+url+'\')">');
	        html.push('    <h3 class="title">'+k.Title+'</h3>');
	        html.push('    '+this._getImg(k.ImageGuids[0]));
	        html.push('    <div class="info-tag">');
	        html.push('      <span>'+k.Media+'</span>');
	...
	getCategory : function(resolve){
		...
		if(data && data.length > 3){
	        ...
	        // static.api.cennoticias.com/category
			// Fetch.getData({url:Country['interface_category'] ,method:'get'},function(data){
