...
getLatestDate = function(){
	// if(window.lan == 'me'){
	// 	window.permission = {'crawl-frequency-chart':1};
	// 	setTimeout(function(){
	// 		const _t = moment().format('YYYY-MM-DD');
	// 		window.lastDate = _t;
	...
	// 	return;
	// }
	API.getLatestDate().then((k) => {
		const _t = moment(k[0].date ,'YYYYMMDD').format('YYYY-MM-DD');
		window.lastDate = _t;
		window.lastTime = new Date(_t).getTime();
		window.startDay = moment(window.lastTime).subtract(7, 'days').format('YYYY-MM-DD');
		window.Loading.hide();
		start();
	});
}
...
console.log('defaultLanguage:',_p,defaultLanguage);
