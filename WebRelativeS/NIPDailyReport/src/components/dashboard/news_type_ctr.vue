<template>
    <tb-panel>
        <div slot="title"><a href="javascript:window.toLink('/news_type_ctr_chart');">新闻类型点击率统计</a></div>
        <div slot="table">
            <i-table 
...
<script>
    ...
    export default {
        ...
        data (e) {
            return {
                columns: [
                    {title: '新闻类型', key: '_name',sortable: 'custom' ,render(r){
                        return `<a href="javascript:window.toLink('/news_type_ctr_chart?c=${r._name}')">${r._name}</a>`;
                    }},
                    {title: '展示数', key: 'newsTypeFetch' ,sortable: 'custom'},
                    {title: '阅读数', key: 'newsTypeRead' ,sortable: 'custom'},
