...
var Util = {
	...
	mergeArrayObject(a ,b ,keyname='key'){
		let _objm = {};

		a.map((k) => _objm[k[keyname]] = k );
		b.map((k) => {
			if(!_objm[k[keyname]]) a.push(k);
		});
	...
	getBxTimeCode(){
		...
		if(year == 2018){
			if(current.isBefore('2018-02-17') || current.isAfter('2018-10-21')){
				return '-02:00';
			}else{
				return '-03:00';
			}
			...
			if(current.isBefore('2018-02-16') || current.isAfter('2018-10-20')){
				return '-02:00';
			}else{
				return '-03:00';
			}
		}else if(year == 2020){
			if(current.isBefore('2018-02-15') || current.isAfter('2018-10-18')){
				return '-02:00';
			}else{
				return '-03:00';
			}
...
window.getBxTimeCode = Util.getBxTimeCode();
...
document.addEventListener('keydown', function(e){
	if(e.which == 27){
		if(typeof(window.onEsc) != 'undefined') window.onEsc();
	}
}, false);
