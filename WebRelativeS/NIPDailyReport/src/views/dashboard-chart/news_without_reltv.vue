    ...
    <layout-main>
        <query-layout slot="main">
            <div slot="condition" class="condition">  
                <span class="title">除去相关新闻数据统计 - 图表</span>
    ...
    export default {
        data(){
            return {
                ...
                chartTitle : '',
                mapping : [
                    { key:'displayNewsWithoutRelative'      , text: '展示新闻数'      , color:'#993333'},
                    { key:'pullNewsCount'    , text: '总刷新次数'    , color:'#CCCC00'},
                    { key:'trendingImpression'       , text: 'Trending展示'       , color:'#663366'},
                    { key:'trendingClick'       , text: 'Trending点击'      , color:'#336633'},
                ],
                comparison : false,
                charts : {
                    checked : ['displayNewsWithoutRelative'],
                    width : _config.width ,
        ...
        methods : {
            ...
            async onSearch(){
                ...
                const e = await API.newsWithoutRelative(range[0] ,range[1]);
                this.data = e.data;
