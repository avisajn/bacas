    ...
    export default {
        data(){
            return {
                ...
                chartTitle : '',
                mapping : [
                    { key:'categoryTagFetch'       , text: '展示数'       , color:'#993333'},
                    { key:'percent'       , text: '算法占比'      , color:'#CCCC00'},
                    { key:'categoryTagRead'       , text: '阅读数'      , color:'#663366'},
                    { key:'ctr'       , text: '点击率'      , color:'#336633'},
                ],
        ...
        methods : {
            ...
            async onSearch(){
                ...
                const e = await API.ctr(range[0] ,range[1]);


                const {columnMapping ,first} = ChartUtil.getColumnMapping(e.data[0]);
                // console.log(`old:${this.columnMapping.length} , new:${columnMapping.length}`);
                this.columnMapping = Util.mergeArrayObject(this.columnMapping ,columnMapping);
                // console.log(`all:${this.columnMapping.length}`);
